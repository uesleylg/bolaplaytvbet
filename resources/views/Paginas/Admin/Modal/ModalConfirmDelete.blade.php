  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg" style="border-radius: 12px; background-color: #0f172a; color: #e2e8f0;">
      ...
      <div class="modal-body text-center">
        ...
        <p class="fs-5">Tem certeza que deseja excluir o usuário <strong id="deleteUserName" class="text-warning"></strong>?</p>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  confirmDeleteBtn.addEventListener('click', async () => {
    ...
    try {
      const deleteUrl = "{{ route('admin.usuario.destroy', ':id') }}".replace(':id', selectedUserId);

      const response = await axios.post(deleteUrl, null, {
        headers: {
          'X-CSRF-TOKEN': csrfToken,
        },
        params: { _method: 'DELETE' } // Laravel aceita DELETE assim
      });
      ...
      if (response.data.success) {
        ...
        alert(response.data.message || 'Erro ao excluir o usuário.');
      }
